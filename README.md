...
docker-compose up -d  -- abre a porta

docker exec -it postgres_container psql -U root -d bd_aula01 -- acessa o banco

docker ps (esse é pra checar se tá rodando)

docker stop "id do docker" -- para ele

docker ps -a mostra todos os dockers abertos

\dt ver as tabelas criadas
...
SELECT * FROM participante; busca tudo
...
SELECT disciplina, aluno_nome, turma, nota FROM notas_alunos;
...
SELECT 
    disciplina, AVG(nota) AS media_notas
FROM    
    notas_alunos
GROUP BY 
    disciplina;
